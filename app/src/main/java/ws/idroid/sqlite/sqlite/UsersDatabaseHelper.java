package ws.idroid.sqlite.sqlite;
import ws.idroid.sqlite.sqlite.User;
...
public class UsersDatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean insertFavourite(String username,Favourites favourite){
        SQLiteDatabase sql = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(KEY_USERNAME,username);
        contentValues.put(KEY_FAVOURITE,favourite.getFavourite());
        contentValues.put(KEY_FAVOURITE_DESCRIPTION,favourite.getDescription());
        sql.insert(TABLE_USERS_FAVOURITE,null,contentValues);
        sql.close();
        return true;
    }

    public ArrayList<Favourites> showAllFavourite (String username){//id of user
        SQLiteDatabase sql = this.getReadableDatabase();
        ArrayList<Favourites> arrayList = new ArrayList<>();
        Cursor cursor =sql.rawQuery("SELECT * FROM "+TABLE_USERS_FAVOURITE+" WHERE USERNAME=? ", new String[]{username});
